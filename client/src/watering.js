...
let print = function() {
  ...
  //  util.deleteOldRows(flowersDiv);
    ...
    flowersArray.forEach(function(item, i, arr) {
        ...
        let waterButton = util.createOperationButton(item, "water-button", "Water this flower", true);
        flowerRow.appendChild(waterButton);

        let deleteButton = util.createOperationButton(item, "delete-button", "Remove flower");
...
let checkFlowerStatuses = function() {
    flowersArray.forEach(function(item, i) {
        ...
        if (util.isDriedUp(item)) {
            util.markWateringButton(rowToMark, util.DRIED_FLOWER_TEXT);
        }
    });
}
    ...
    flowersArray.forEach(function(item, i) {
        if (item.name === name) {
            ...
            if (util.isOverWatered(item)) {
                util.markWateringButton(rowToMark, OVER_WATERED_FLOWER_TEXT);
            }
...
let setOnAddFlowerSubmitEvent = function() {
    ...
    submitAddFlower.addEventListener("click", function() {
        ...
        util.hideElement("add-flower-form");
...
let registerAddFlowerListener = function() {
    ...
    addFlowerButton.addEventListener("click", function() {
        util.showElement("add-flower-form");
    }, true);
